...
def analyze_vnt_sentiment():
    ...
    for vnt_dir in VNT.iterdir():
        ...
        for chat_name in tp["per_contact"].keys():
            ...
            if (
                chat_lower == contact_lower
                or chat_lower in contact_lower
                or contact_lower in chat_lower
            ):
                # Get tier from rec
    ...
    # Most emotional
    emotional = sorted(
        [(c, info) for c, info in by_contact.items()], key=lambda x: -x[1]["emotional_word_count"]
    )[:10]
