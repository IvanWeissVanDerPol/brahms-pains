...
def main():
    ...
    for t in valid:
        ...
        if s > 0.2:
            sent_buckets["positive"] += 1
        elif s < -0.2:
            sent_buckets["negative"] += 1
        else:
            sent_buckets["neutral"] += 1
    ...
    print()
    print(
        f"Total: {out['total_transcripts']:,} transcripts, {out['total_words']:,} words, {out['total_duration_hours']:.1f}h audio"
    )
