...
def main():
    ...
    for source_jid, mentioned in cross_refs.items():
        for name, count in mentioned.items():
            ...
            if len(edge_evidence[(a, b)]) < 3:
                edge_evidence[(a, b)].append(f"{jid_to_name[a]} ↔ {jid_to_name[b]}: {count} cross-refs")
    ...
    for (a, b), weight in sorted_edges:
        if weight >= MERGE_THRESHOLD and find(a) != find(b):
            union(a, b)
            merged += 1
            if merged > 500: break  # don't merge everything
    ...
    for i, cluster in enumerate(cluster_list):
        cluster_out.append({
            ...
            "internal_edges": sum(edges.get(tuple(sorted([a, b])), 0) for a in cluster for b in cluster if a < b),
        })
