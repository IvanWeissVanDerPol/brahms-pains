# Messaging Backup Manifest

Snapshot import of Ivan's Messaging corpus into this repo for psychoanalysis work.
...
Re-extraction requires the phone backup + the Messaging backup key.
...
| server | chats |
|--------|------:|
| `s.messaging.net` (1:1 phone) | 9,835 |
...
`.amr`) copied from `/data/media/phone-extract/wa/Media/Messaging Voice Notes/` and
`.../Messaging Audio/`, keyed by chat slug so the audio is co-located with the text
timeline. `_w4b_unmapped/` holds Messaging Business audios whose DB was not
...
2. Decrypt with the Messaging backup key → `msgstore-latest-2026-07-20T14.db`.
