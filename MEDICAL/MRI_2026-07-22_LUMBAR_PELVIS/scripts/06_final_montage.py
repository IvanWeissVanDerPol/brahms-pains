...
ax.set_title(
    "COR T2 ASYMMETRY\nRED = patient RIGHT brighter\n(consistent with right-sided inflammation)",
    fontsize=11,
)
ax.axis("off")
...
ax.set_title(
    f"AX T1 — slice {si_z_ax} (loc {locs_ax_t1[si_z_ax]:.1f} mm)\nSI joint level", fontsize=12
)
ax.axis("off")
...
asym = json.load(
    open(
        "/root/psycology/MEDICAL/MRI_2026-07-22_LUMBAR_PELVIS/analysis/muscle_asymmetry_ax_stir.json"
    )
)
...
ax.fill_between(
    locs, rights, lefts, where=lefts > rights, color="red", alpha=0.3, label="RIGHT side hotter"
)
ax.set_title(
    "Pelvic muscle T2-STIR asymmetry per axial slice\n48% of slices show patient RIGHT > LEFT (consistent inflammation pattern)",
    fontsize=12,
)
...
plt.suptitle(
    "MRI LUMBAR + PELVIS — IVAN 2026-07-22 — KEY FINDINGS MONTAGE\n26yo M, RIGHT buttock/cintura/hip pain of unclear origin",
    fontsize=15,
    fontweight="bold",
    y=0.995,
)
