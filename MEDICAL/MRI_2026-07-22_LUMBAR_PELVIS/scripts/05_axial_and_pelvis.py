...
print(f"Scanning inferior slices (scrotal region)...")
...
for z in sorted(slice_clusters.keys())[-5:]:  # last 5 slices
    ...
    for c in clusters[:4]:
        ...
        print(f"    {side:>5} cluster @ ({c['centroid_x']:>3}, {c['centroid_y']:>4})  size={c['size_px']:>5} px  ({c['w_px']}x{c['h_px']} px)")
...
for c in slice_clusters[best_z]:
    cluster_mask = (labeled == (np.abs(labeled - c['centroid_y']) < 50)[:, :]) & (np.abs(np.arange(sl_n.shape[1])[None, :] - c['centroid_x']) < 50)
...
if len(clusters_b) >= 2:
    ...
    for i, c in enumerate(sorted_by_x[:2]):
        ...
        axes[2].text(c['centroid_x'], c['centroid_y'], f'{side_label}\n{c["size_px"]} px',
                     color=color, fontsize=11, fontweight='bold', ha='center',
                     bbox=dict(boxstyle='round,pad=0.3', facecolor='black', alpha=0.6))
    axes[2].set_title('Bilateral scrotal fluid collections\nRED = patient RIGHT hydrocele', fontsize=12)
    axes[2].axis('off')
...
for z in range(ax_stir.shape[0]):
    ...
    asym_per_slice.append({'z': z, 'loc_mm': float(stir_locs[z]),
                            'left': l_mean, 'right': r_mean, 'diff': diff})
...
for name, lo, hi in sup_slabs:
    if hi > lo:
        ...
        print(f"  {name:<40} {ml:>8.4f} {mr:>8.4f} {diff:>+9.4f}  → {side} side hotter by {abs(ml - mr):.4f}")
...
ax.fill_between(locs_a, right_v, left_v, where=left_v > right_v, color='red', alpha=0.3, label='patient RIGHT side hotter')
ax.fill_between(locs_a, left_v, right_v, where=right_v > left_v, color='blue', alpha=0.3, label='patient LEFT side hotter')
ax.set_title('Pelvic muscle T2-STIR asymmetry (per axial slice)\nRED shaded region = patient RIGHT side is brighter (=more edema)', fontsize=14)
