...
for z in range(cor_t1.shape[0]):
    ...
    results.append({
        ...
        'L_ilium_t1': L_ilium_t1, 'L_ilium_t2': L_ilium_t2,
        'L_sacrum_t1': L_sacrum_t1, 'L_sacrum_t2': L_sacrum_t2,
        'R_ilium_t1': R_ilium_t1, 'R_ilium_t2': R_ilium_t2,
        'R_sacrum_t1': R_sacrum_t1, 'R_sacrum_t2': R_sacrum_t2,
    })
...
print()
print(f"{'z':>3} {'loc':>7} {'L_il_T2':>7} {'L_sc_T2':>7} {'R_il_T2':>7} {'R_sc_T2':>7} {'R-L diff':>8}  {'R > L by':>7}")
for r in results:
    rt2 = (r['R_ilium_t2'] + r['R_sacrum_t2']) / 2
    lt2 = (r['L_ilium_t2'] + r['L_sacrum_t2']) / 2
    diff = rt2 - lt2
    flag = '  ★' if diff > 0.05 else ''
    print(f"{r['z']:>3} {r['loc_mm']:>7.1f} {r['L_ilium_t2']:>7.3f} {r['L_sacrum_t2']:>7.3f} {r['R_ilium_t2']:>7.3f} {r['R_sacrum_t2']:>7.3f} {diff:>+8.3f}{flag}")
...
print()
print(f"=== Summary across all coronal slices ===")
...
right_brighter = sum(1 for r in results if (r['R_ilium_t2'] + r['R_sacrum_t2']) > (r['L_ilium_t2'] + r['L_sacrum_t2']) + 0.05)
...
if right_mean_t2 - left_mean_t2 > 0.05:
    summary['interpretation'] = 'RIGHT SI joint shows increased fluid signal vs LEFT — suggests RIGHT-sided bone marrow edema (BME).'
...
axes[0].fill_between(locs_arr, l_t2, r_t2, where=r_t2 > l_t2, color='red', alpha=0.2, label='RIGHT > LEFT (BME)')
axes[0].set_title('SI Joint Subchondral T2-WATER intensity per slice\n(Higher = more fluid/edema = active sacroiliitis feature)', fontsize=14)
...
axes[1].fill_between(locs_arr, l_t1, r_t1, where=r_t1 > l_t1, color='green', alpha=0.2, label='RIGHT > LEFT (fat)')
axes[1].set_title('SI Joint Subchondral T1 intensity per slice\n(Higher T1 = fat metaplasia = chronic sacroiliitis feature)', fontsize=14)
