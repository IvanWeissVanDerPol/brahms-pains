...
for z in range(cor_t1.shape[0]):
    ...
    right_joint_zone = slice(min(cor_t1.shape[2], sacrum_right + 2), min(cor_t1.shape[2], sacrum_right + 25))
    ...
    w = 8
    R_si_il_t1 = sl_t1[best_row-w*2:best_row+w*2, max(0, right_joint_col):right_joint_col+w].mean()
    R_si_sc_t1 = sl_t1[best_row-w*2:best_row+w*2, right_joint_col:min(cor_t1.shape[2], right_joint_col+w)].mean()
    L_si_il_t1 = sl_t1[best_row-w*2:best_row+w*2, max(0, left_joint_col-w):left_joint_col].mean()
    L_si_sc_t1 = sl_t1[best_row-w*2:best_row+w*2, left_joint_col:min(cor_t1.shape[2], left_joint_col+w)].mean()

    R_si_il_t2 = sl_t2[best_row-w*2:best_row+w*2, max(0, right_joint_col):right_joint_col+w].mean()
    R_si_sc_t2 = sl_t2[best_row-w*2:best_row+w*2, right_joint_col:min(cor_t1.shape[2], right_joint_col+w)].mean()
    L_si_il_t2 = sl_t2[best_row-w*2:best_row+w*2, max(0, left_joint_col-w):left_joint_col].mean()
    L_si_sc_t2 = sl_t2[best_row-w*2:best_row+w*2, left_joint_col:min(cor_t1.shape[2], left_joint_col+w)].mean()
    ...
    all_findings.append({
        ...
        'R_si_il_t1': float(R_si_il_t1), 'R_si_sc_t1': float(R_si_sc_t1),  # patient RIGHT
        'L_si_il_t1': float(L_si_il_t1), 'L_si_sc_t1': float(L_si_sc_t1),  # patient LEFT
        'R_si_il_t2': float(R_si_il_t2), 'R_si_sc_t2': float(R_si_sc_t2),
        'L_si_il_t2': float(L_si_il_t2), 'L_si_sc_t2': float(L_si_sc_t2),
    })
...
print()
print(f"=== RIGHT (PATIENT RIGHT) vs LEFT SI joint summary ===")
print(f"  T2-WATER:  RIGHT={right_t2.mean():.4f}  LEFT={left_t2.mean():.4f}  diff={right_t2.mean()-left_t2.mean():+.4f}")
print(f"  T1:        RIGHT={right_t1.mean():.4f}  LEFT={left_t1.mean():.4f}  diff={right_t1.mean()-left_t1.mean():+.4f}")
print()
print(f"  Slices where RIGHT T2 > LEFT T2 by >0.03: {(right_t2 > left_t2 + 0.03).sum()}/{len(all_findings)}")
print(f"  Slices where RIGHT T2 > LEFT T2 by >0.05: {(right_t2 > left_t2 + 0.05).sum()}/{len(all_findings)}")
print(f"  Slices where RIGHT T1 > LEFT T1 by >0.05: {(right_t1 > left_t1 + 0.05).sum()}/{len(all_findings)}")
...
for idx, sample_i in enumerate(sample_idx):
    ...
    axes[idx, 0].set_title(f'COR T1 z={z} loc={f["loc_mm"]:.1f}mm\nSI joints (red lines)', fontsize=11)
    axes[idx, 0].axis('off')
    ...
    axes[idx, 1].set_title(f'COR WATER T2 same slice\nR T2={f["R_si_il_t2"]:.3f} L T2={f["L_si_il_t2"]:.3f}', fontsize=11)
    axes[idx, 1].axis('off')
...
axes[0].fill_between(locs_arr, 0, diff, where=diff > 0, color='red', alpha=0.3, label='RIGHT > LEFT')
axes[0].fill_between(locs_arr, diff, 0, where=diff < 0, color='blue', alpha=0.3, label='LEFT > RIGHT')
axes[0].set_title(f'SI Joint Subchondral T2-WATER (BME detection)\nRIGHT mean={right_t2.mean():.4f}, LEFT mean={left_t2.mean():.4f}, diff={right_t2.mean()-left_t2.mean():+.4f}', fontsize=13)
...
axes[1].fill_between(locs_arr, 0, diff_t1, where=diff_t1 > 0, color='red', alpha=0.3, label='RIGHT > LEFT')
axes[1].fill_between(locs_arr, diff_t1, 0, where=diff_t1 < 0, color='blue', alpha=0.3, label='LEFT > RIGHT')
axes[1].set_title(f'SI Joint Subchondral T1 (fat metaplasia detection)\nRIGHT mean={right_t1.mean():.4f}, LEFT mean={left_t1.mean():.4f}, diff={right_t1.mean()-left_t1.mean():+.4f}', fontsize=13)
