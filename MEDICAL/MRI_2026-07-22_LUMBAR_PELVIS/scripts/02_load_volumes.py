...
def load_volume(insts_sorted, sn, desc, bvalue=None):
    ...
    info = {
        ...
        'dtype': str(vol.dtype),
        'slice_loc_min': float(min(slice_locs)) if slice_locs and slice_locs[0] is not None else None,
        'slice_loc_max': float(max(slice_locs)) if slice_locs and slice_locs[0] is not None else None,
        'voxel': {
            'pixel_mm_x': insts_sorted[0]['pixel_mm'][1] if len(insts_sorted[0]['pixel_mm']) > 1 else None,
            'pixel_mm_y': insts_sorted[0]['pixel_mm'][0] if len(insts_sorted[0]['pixel_mm']) > 0 else None,
...
# Load the index
idx = json.load(open("/root/psycology/MEDICAL/MRI_2026-07-22_LUMBAR_PELVIS/analysis/series_index_full.json"))
manifest = {}

for sn_str, insts in idx.items():
    sn = int(sn_str)
    insts_sorted = sorted(insts, key=lambda x: (x['slice_loc'] if x['slice_loc'] is not None else 0))
    ...
    if 'DWI' in desc:
        ...
        for bv, group in bv_groups.items():
            group_sorted = sorted(group, key=lambda x: (x['slice_loc'] if x['slice_loc'] is not None else 0))
            info = load_volume(group_sorted, sn, desc, bvalue=bv)
            manifest[f"s{sn}_b{int(bv)}"] = info
            print(f"  Series {sn} DWI b={bv}: {info['n_slices']} slices, {info['file_size_mb']}MB, I=[{info['intensity']['p1']:.0f}..{info['intensity']['p99']:.0f}]")
    else:
        info = load_volume(insts_sorted, sn, desc)
        manifest[f"s{sn}"] = info
        print(f"  Series {sn:>5} {desc:<30}: {info['n_slices']:>3} slices, {info['file_size_mb']:>5}MB, I=[{info['intensity']['p1']:.0f}..{info['intensity']['p99']:.0f}]")
