# Family-of-Origin Map — 2026-08-27 Cast

**Corrected relational map (per Iván, 2026-08-28):**
- **Iván + Ale** = co-residents (the only household pair)
- **Iván + Kiki** = siblings (real family)
- **Iván + Gaby** = friends (not household)
- **Iván + Lourdes** = friends (therapist-in-training, clinical frame not announced in transcript)
- **Cuqui** = absent subject (2nd Cuqui session)

**Implication:** The household system is **smaller and tighter** than I first mapped. Iván is **the hub** — three of the five in-room people (Ale, Kiki, Gaby) and the absent subject (Cuqui) all connect through him. Lourdes is **the only** non-Iván-mediate relation in the room (Lourdes ↔ Gaby is the secondary link).

This file maps the **observable family-of-origin material** for each speaker. **Most speakers did NOT surface explicit origin material in this session** — what follows is what surfaced plus inferences.

**Enriched:** 2026-08-28 with TREATMENT/CASE_CONCEPTUALIZATION.md + TREATMENT/CLINICAL_SUMMARY.md + 20hats analysis (full family-of-origin context for Iván documented)

---

## The Weiss-Van der Pol family (Iván's origin) — full map

**Source:** `TREATMENT/CASE_CONCEPTUALIZATION.md` (Jan 2026 clinical summary by prior psychologist)

**Family unit:**
- **Father:** John Weiss ("Do Not Disturb" — minimal emotional engagement, emotionally unavailable)
- **Mother:** Sonia Weiss ("Unpredictable Manager" — high variance between warmth and criticism)
- **Sister:** Kyrian Weiss (= Kiki, the elder sister) — closest family but still compartmentalized
- **Brother:** Gonzalo Weiss (middle? — relationship unclear in corpus)
- **Iván** — middle child between Kyrian and Gonzalo
- **Grandfather:** Jan Van der Pol (60 messages in corpus, low intensity)

**Origin wound — the "pesado" labeling:**
- **Age 2+:** Family-wide labeling as **"inchabola"** (ball-buster/annoying) and **"pesado"** (heavy/burdensome) as persistent descriptors
- Natural childhood exuberance (loud, intense, needy) met with family-wide rejection
- **Internalized message:** "My natural emotional needs make me burdensome. To belong, I must suppress needs and provide value."
- **Behavioral adaptation:** Compulsive service-giving (The Fixer) as earning strategy
- **Neurobiological impact:** Hypervigilance to rejection cues, chronic stress response (service compulsion), touch starvation (physical affection withdrawn), emotional compartmentalization (The Firewall)

**Cultural layer:**
- Dutch-Paraguayan bicultural background
- Class mobility (middle to upper-middle)
- Paraguayan heteronormative culture as contrast anchor
- Sexuality and gender expression constrained

**Family Member Map (per TREATMENT):**
- **Kyrian (sister):** Closest, but still compartmentalized. **7-year stable relationship, 7,838 messages, balanced 49.1% initiation.**
- **Mother/Father:** "Don't disturb" / "Unpredictable manager." **Mom at 50.3% initiation, 11,305 messages, 7-year stable. Dad at 23% initiation, 1,864 messages, 7-year stable but 0% emotional content.**
- **Gonzalo (brother):** Relationship unclear in corpus. **Not in tier1.**
- **Grandpa Jan:** Low-intensity contact. **60 messages, distant.**

**Corpus-wide attachment distribution (per HAT 1/4 validation):**
- **Reciprocal balanced (12 chats):** Mom, Kiki, Grandma Riet — these are the secure-base relationships
- **Pursued by partner (anxious-preoccupied):** Gaby, Jonatan, Lourdes (post-reunion)
- **Pursuer (deactivating):** Magali, Nico, Thais, Sarah
- **Absent:** Dad (0% affection, 0% distress, 0% boundary content)

**The bimodality maps directly onto origin wound:**
- Iván pursues OR is pursued because he cannot hold balanced intimacy with primary attachment figures — only **with caregivers (Mom/Kiki/Grandma) and recent high-investment partners (Lourdes post-594d gap, Gaby 53d)** does he approach balance
- The compensatory chosen family (polycule, AIW org, 20 hats, this session) **rebuilds the secure base** his origin family didn't provide

**HIV-positive disclosure (Jan 2026 breakthrough):**
- Per CASE_CONCEPTUALIZATION: HIV status hidden from all family (the Firewall)
- Jan 2026: **proactive HIV disclosure** — first time Ivan broke through the Firewall to be fully authentic with a primary partner
- This is the **earned-secure** marker — Ivan is **halfway through**, origin work is deferred to chosen family but no longer fully compartmentalized

**What this means for the 08-27 reading:**
- The session is **not** about Iván's origin family. The origin family is **largely absent** from the conversation — Ivan is processing the household-of-choice.
- The 08-27 "no puedo decir que no" pattern is the **operational expression** of the pesado defense organization: the family-wide rejection at age 2 created the Fixer; the Fixer manifests in the household as saying yes to Cuqui
- The polycule structure is the **reparative chosen family** that the origin family couldn't provide
- **The next session's work** (origin family) is not in this room. It's in Lalla's (Ivan's actual therapist) room.

---

## Iván

### What surfaced
- **MRI / cancer scare** (+785s): "que capaz era cáncer, pero al final no era, así que..."
- Origin family is **not detailed** in this session — Iván is **mostly talking about the household-of-choice** (the polycule + friends + sibling)
- "Yo le dije a Iván..." — Iván talks about himself in **3rd person** when processing aloud, suggesting **a habit of observing himself from outside**

### Inferred origin (low confidence)
- Paraguay + Argentina bicultural background (Spanish/English code-switching, Asunción/Buenos Aires references)
- The polycule is **a chosen family structure** — strongly associated with queer/ENM adults whose origin family was rejecting
- The 20 hats project, the 33-repo AIW org, the IR repository — **building structures** to be the **architect** of systems he couldn't control in his origin
- Kiki as sister — **the sibling relationship carries weight** that doesn't get processed in this session

### Process question
"What did your family-of-origin teach you about boundaries? Is the 'no puedo decir que no' pattern from them, or did you build it to compensate?"

---

## Gaby

### What surfaced (substantial — this is the only speaker with detailed origin material)
- **Father died in her presence, suddenly, while she was driving** (+6813s): "Mi papá falleció conmigo a mi lado. O sea, yo estaba manejando y yo iba así, ¡pum! [palmada] se cayó y falleció acá en mi relación."
- **Gaby is RCP-trained** ("profesional de blanco, yo sé hacer RCP") — and **blamed herself for years** for not being able to save him (+6826s)
- **Mother mentioned at +4638s** ("mi mamá") — content cut off
- **Lifelong caregiving**: "viví toda mi vida cuidándola a mi abuela, a mi hermana, a mi papá, a todo el mundo" (+4385s)
- "Mis pobres cerebros" — **plural brain-states**, suggesting she experiences herself as **multiple** (potential dissociative feature, common in caregivers of dying parents)
- "Cornearon asquerosamente" — graphic bodily language about how the trauma was inflicted

### Pattern interpretation
**Gaby's origin story is the canonical parentified-child narrative:**
- Multiple dependents (abuela, hermana, papá, mamá)
- Father dies **without warning**, in her arms
- She is **professionally trained** (RCP) — implies medical/healthcare background, which is **itself** a defensive career choice
- The "too nice / drains energy" pattern is **directly traceable** to: "if I take care of everyone, the next person won't die in my arms"

### Process question (highest priority for next session)
"You've been carrying your father's death for years. What's the relationship between that and 'demasiado dulce'?"

---

## Lourdes

### What surfaced
- The **2-year Dubai silence** (+6394s) — Lourdes cut contact with one member of the household (probably Nate) over a monogamy/non-monogamy conflict
- Names patterns ("demasiado bueno", "demasiado dulce") with **clinical precision** — suggests training
- "Mi personaje de Lourdes" — **self-positions as character/role**, suggesting either acting/performing background or dissociation
- "Lourdes" / "Youko Kurama" — **dual-name** suggests **dual identity** (real name + chosen name), common in queer/trans contexts or in people who had to construct a public self

### What is NOT in the transcript
- **No direct family-of-origin references** in this session
- **No mention of her own family** — she talks about the household exclusively
- The clinical precision **comes from somewhere** — therapist-in-training training (per Iván)

### Pattern interpretation
Lourdes has **successfully professionalized** her psychological insight — she sees patterns, names them, sets limits. But the **2-year exile** is the cost. She has **moved herself to the periphery** of her own relationships in order to keep her clarity.

### Process question
"Where did you learn to see relationships so clearly? Was the 2-year silence the cost of your training, or the cause of it?"

---

## Kiki (Iván's sister)

### What surfaced
- **Sibling language throughout** — "vos tenés que poner límites", "vos también", "vos sos la imagen de la empresa"
- "Tipo conmigo vos no vas a hacer eso" (+3493s) — **the sister contract**: I am the exception to your yes-to-everyone pattern
- "No llegás ni a la cantidad de proteína" (+3542s) — **the sisterly nutrition lecture** (universal sibling trope)
- "Actually cuidarte a vos y dormir a hora, comer bien" (+3542s) — **the older-sister speech about self-care**
- "Pollo egoísta" is implied in the register — **the older sister framing**
- **Argentine professional register** at +3451s onward — code-switches when going into "intervention mode"
- "Petunio" — unclear reference, possibly her own companion (animal? partner? symbolic?)

### Pattern interpretation (high confidence)
**Kiki is the elder sister / caretaker-of-the-brother archetype.** Her **"I cannot fix anybody but myself"** is the elder-sister who has **already done the work** and is now **delivering it as instruction**.

She has integrated two registers:
- **Paraguayan sister** — familiar, calling him "boludo", affectionate roast
- **Argentine professional** — clinical precision, intervention framing

This **dual register** is **a feature, not a pathology** — it's the bilingual home, the bicultural family, the sibling-protective role all at once.

### What is NOT in the transcript
- Direct family-of-origin content — **Kiki doesn't process her own origin in this session, only Iván's**
- "Petunio" is unresolved — could be Kiki's way of having **her own care-object** that she doesn't share

### Process question
"Petunio. And: did you learn to be the older sister because you had to, or because you chose to?"

---

## Ale

### What surfaced
- **Co-resident with Iván** (the only household pair)
- Reports Cuqui's 80 calls at 3am (+3226s) — **active vigilance**
- "Me dejaron afuera de todas las conversaciones" (+3097s) — **excluded from the Cuqui-exit decision**
- Spelled out "polycule" (+487s) — **provides vocabulary for the system**
- Leaves at ~55 minutes

### What is NOT in the transcript
- **No direct family-of-origin references**
- Ale is **the quietest speaker** — origin material likely exists but isn't being shared

### Pattern interpretation
Ale is **the systems observer who lives in the system**. His exclusion from the Cuqui decision **re-enacts a familiar pattern** (the person who sees everything but isn't consulted). His leaving at 55 minutes could be:
- Genuine scheduling
- **Self-removal before being told to leave**
- A signal that the work after that point was **not for him**

### Process question
"You left at 55 minutes. Was there something specific you didn't want to be there for, or something specific you had to do?"

---

## Cuqui (absent)

### What surfaced
- **Aldea SOS background** (+2574s, per Iván): "Cuqui tiene contexto de la aldea SOS, que eran niños que pasaban por abuso y tienen los psicóloga ahí"
- **Abuse history**: "como sufría abuso" (+4524s)
- Has a daughter
- Has been **labeled TLP** by Lourdes, by Cuqui herself, and **disputed** by Manuel
- Bed incident, 80 calls, breaking into houses

### Pattern interpretation (high confidence)
Cuqui's origin is **the canonical trauma-survivor profile**:
- Aldea SOS = residential care for abused children in Paraguay
- Disputed TLP label
- Self-labels TLP ("Cuqui dijo TLP") — **insight without behavioral change**
- Cycles through household placements (was at Gaby's, was at the polycule, was at Nate's, now... ?)

### The daughter's frame
The daughter is **the most under-addressed topic** in the session. Cuqui's acting-out directly affects a child. **This is a child-welfare frame**, not just a clinical frame.

### Process question
"If Cuqui is in next session, the first 15 minutes need to be: 'what does Sofita need?' (or whatever the daughter's name is)."

---

## System-wide origin pattern

| Speaker | Origin wound | Household echo |
|---|---|---|
| Iván | Unspecified; possibly bicultural/rejecting origin | Chosen family (polycule) as compensation |
| Gaby | Father died in her arms + lifelong parentification | Compulsive caregiving + receiving the household's care |
| Lourdes | Unspecified origin; professionalization of insight | 2-year exile as the cost of clarity |
| Kiki | Older sister role (chosen or required) | Sibling-as-therapist for the household |
| Ale | Unspecified origin; pattern of being left out | Reports from the periphery, leaves before being left out |
| Cuqui | Aldea SOS + abuse history | Acting-out, splitting, safety issues |

**The room is full of people who had to take care of themselves early.** This is **a household of self-made adults** — each of them built their own structure because origin family didn't provide one. Cuqui, who had **the least structural support** (Aldea SOS, abuse history), is **the one the system can't hold**.

---

## Intervention frame

For each speaker, the question is: **"What did your family-of-origin teach you to be, and is that still working for you?"**

- **Iván**: Chosen family is functional. Origin work is deferred.
- **Gaby**: Caregiving was a survival strategy. **It's no longer survival** — but the body still runs it as if it were.
- **Lourdes**: Professionalization was a survival strategy. **The 2-year exile** suggests the cost was high.
- **Kiki**: Older-sister role was a survival strategy. **"I cannot fix anybody but myself"** is the integration — now the question is what she wants to do with herself.
- **Ale**: Peripheral observation was a survival strategy. **The 80 calls reporting** is functional; the leaving is the symptom.
- **Cuqui**: Aldea SOS was a survival strategy. **Acting out was a survival strategy.** The strategies are still running. The question is whether she can build new ones.

---

*File: `PSY_FAMILY_ORIGINS.md`*
*Source: transcript + Iván's corrections (Lourdes = therapist-in-training, Kiki = Iván's sister, Ale = Iván's roommate, Gaby = friend)*
