# Mario Gueyraud

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595991705424
> **Tier:** tier3_extended · **Relationship score:** 49.1 (WARM)
> **Stats:** 703 messages · 2021-10-12 → 2023-10-26

## Overview

**Auto-extracted stats** (348 from Ivan, 355 from them, 6 emojis)

Peak months: `2021-10` (288 msgs), `2021-11` (151 msgs), `2022-05` (47 msgs)
Language mix: es: 128, other: 571, en: 3, mixed: 1
Most active: **Tuesday** at **0:00** (evening (18-23))
Top topics: hacer (18), jajaja (16), estoy (16), mismo (16), ahora (15)
Avg msg length: Ivan 37 chars, them 20 chars
Avg reply time: Ivan 6m, them 6m
Question ratio: Ivan 9.8%, them 14.4%
Longest streak: **3** consecutive days · Longest gap: **347** days
Audio usage: 4.1% of all messages are voice notes
Sentiment: Ivan -0.09 (slightly negative) (pos 5 / neg 6), them +0.14 (positive) (pos 4 / neg 3)
Mood trend: overall +0.02, recent -0.33, trend Δ -0.78 (-0.78 (cooling))

**They call Ivan:** 'loco' (1x)

**First message** (2021-10-12, Ivan): Hola

**Last from Ivan** (2023-10-26): re mood

**Last from them** (2023-10-26): Xd

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 703 |
| First message | 2021-10-12 |
| Last message | 2023-10-26 |

## Key moments / Topics

Auto-extracted notable messages from the chat:

## Notable messages (auto-extracted)

- **[2021-10-12] Ivan** (0 emojis): Bien random Pero soy luego random  Básicamente me interesa trabajar para el exterior también  Y esa wea Pero es muy xd hacerse amigo de alguien por algo asi nomas muy hecho puta/manipulador  Asi que hola mario Soy ivan weiss Via tratar de hacerme tu amigo para saber como conseguiste y eso y no se se
- **[2021-10-12] Ivan** (0 emojis): ahora se estaba quejando por que su hierba no era como el queria todos los que probaron estaban onda  hdp sii pero el pichado por que tuvo semillas por que una planta se volvio hermafrodita por que el ciclo de dia noche estaba mal y por que la planta tenia semillas no se fue el 100% de la energia en
- **[2021-10-13] Them** (0 emojis): Escriba un script que tome exactamente un argumento, un nombre de directorio, tal que: (a) Si el número de argumentos es diferente de uno, imprima un mensaje de uso. (b) Si el argumento no es un directorio, que imprima un mensaje de error informativo. (c) Para el directorio dado, liste los cinco arc
- **[2021-10-20] Them** (0 emojis): #!/bin/bash #Script "service.sh" #Probar con: sudo bash service.sh initd INICIAR HParámetros posicionales DEMONIO=$1 #Acciones disponibles: INICIAR, PARAR, REINICIAR ACC=$2 ARCHIVOPID="/proc/run/demonio.pid" start() { DEMONIO=$1 ARCHIVOPID=$2 PID= su -c "nohup SDEMONIO >> /var/log/demonio.log&" SPID
- **[2021-11-17] Ivan** (0 emojis): a-) if [ -d "/path/dirección" ]; then            echo "/path/dirección. is a valid and existing path" fi  b-)  if [ -d direccion ]; then            echo "es un directorio" elif [ -f direccion ]; then            echo "es un archivo" else            echo "es otro tipo de archivo" fi  ese lo que tengo 


## Notable messages (auto-extracted)

- **[2021-10-12] Ivan** (0 emojis): Bien random Pero soy luego random  Básicamente me interesa trabajar para el exterior también  Y esa wea Pero es muy xd hacerse amigo de alguien por algo asi nomas muy hecho puta/manipulador  Asi que hola mario Soy ivan weiss Via tratar de hacerme tu amigo para saber como conseguiste y eso y no se se
- **[2021-10-12] Ivan** (0 emojis): ahora se estaba quejando por que su hierba no era como el queria todos los que probaron estaban onda  hdp sii pero el pichado por que tuvo semillas por que una planta se volvio hermafrodita por que el ciclo de dia noche estaba mal y por que la planta tenia semillas no se fue el 100% de la energia en
- **[2021-10-13] Them** (0 emojis): Escriba un script que tome exactamente un argumento, un nombre de directorio, tal que: (a) Si el número de argumentos es diferente de uno, imprima un mensaje de uso. (b) Si el argumento no es un directorio, que imprima un mensaje de error informativo. (c) Para el directorio dado, liste los cinco arc
- **[2021-10-20] Them** (0 emojis): #!/bin/bash #Script "service.sh" #Probar con: sudo bash service.sh initd INICIAR HParámetros posicionales DEMONIO=$1 #Acciones disponibles: INICIAR, PARAR, REINICIAR ACC=$2 ARCHIVOPID="/proc/run/demonio.pid" start() { DEMONIO=$1 ARCHIVOPID=$2 PID= su -c "nohup SDEMONIO >> /var/log/demonio.log&" SPID
- **[2021-11-17] Ivan** (0 emojis): a-) if [ -d "/path/dirección" ]; then            echo "/path/dirección. is a valid and existing path" fi  b-)  if [ -d direccion ]; then            echo "es un directorio" elif [ -f direccion ]; then            echo "es un archivo" else            echo "es otro tipo de archivo" fi  ese lo que tengo 


## Notable messages (auto-extracted)

- **[2021-10-12] Ivan** (0 emojis): Bien random Pero soy luego random  Básicamente me interesa trabajar para el exterior también  Y esa wea Pero es muy xd hacerse amigo de alguien por algo asi nomas muy hecho puta/manipulador  Asi que hola mario Soy ivan weiss Via tratar de hacerme tu amigo para saber como conseguiste y eso y no se se
- **[2021-10-12] Ivan** (0 emojis): ahora se estaba quejando por que su hierba no era como el queria todos los que probaron estaban onda  hdp sii pero el pichado por que tuvo semillas por que una planta se volvio hermafrodita por que el ciclo de dia noche estaba mal y por que la planta tenia semillas no se fue el 100% de la energia en
- **[2021-10-13] Them** (0 emojis): Escriba un script que tome exactamente un argumento, un nombre de directorio, tal que: (a) Si el número de argumentos es diferente de uno, imprima un mensaje de uso. (b) Si el argumento no es un directorio, que imprima un mensaje de error informativo. (c) Para el directorio dado, liste los cinco arc
- **[2021-10-20] Them** (0 emojis): #!/bin/bash #Script "service.sh" #Probar con: sudo bash service.sh initd INICIAR HParámetros posicionales DEMONIO=$1 #Acciones disponibles: INICIAR, PARAR, REINICIAR ACC=$2 ARCHIVOPID="/proc/run/demonio.pid" start() { DEMONIO=$1 ARCHIVOPID=$2 PID= su -c "nohup SDEMONIO >> /var/log/demonio.log&" SPID
- **[2021-11-17] Ivan** (0 emojis): a-) if [ -d "/path/dirección" ]; then            echo "/path/dirección. is a valid and existing path" fi  b-)  if [ -d direccion ]; then            echo "es un directorio" elif [ -f direccion ]; then            echo "es un archivo" else            echo "es otro tipo de archivo" fi  ese lo que tengo 


## Notable messages (auto-extracted)

- **[2022-03-09] Ivan** (0 emojis): Holis Que tal?  Vos estás por poli?  Estoy aplicando a unas becas a Canadá Y pedí unos doscientos por e trámite  Y al parecer hay que retirar de la facultad y no sé cuando voy a ten cuidado tiempo de ir   Y te quiero preguntar si podes por si acaso retirar por mi y mandarme onda camscanner el doc pa
- **[2021-11-17] Them** (0 emojis): Cuando un proceso pesado hijo es generado, el mismo hereda los descriptores de archivos de su proceso padre. Indique: (a) ¿Qué pasará en el proceso padre si el proceso hijo cierra el descriptor de archivo?. (b) ¿Qué pasará si ambos procesos, padre e hijo, escriben simultáneamente al archivo?
- **[2023-10-26] Ivan** (0 emojis): basicamente escribir documentos y verificar cosas onda el boton esta? hacer click anda?  ahora estoy hacienda cursos para automatizar como MM quiere pero por mi cuenta se hacer en python ya
- **[2022-09-23] Ivan** (0 emojis): hola que tal?  vi que hoy hay un evento de progrmacion con robots que vas a hacer que maso van a hacer? suena interesnte  tengo un proyecto que quiero hacer con robotica


## Notable messages (auto-extracted)

- **[2022-03-09] Ivan** (0 emojis): Holis Que tal?  Vos estás por poli?  Estoy aplicando a unas becas a Canadá Y pedí unos doscientos por e trámite  Y al parecer hay que retirar de la facultad y no sé cuando voy a ten cuidado tiempo de ir   Y te quiero preguntar si podes por si acaso retirar por mi y mandarme onda camscanner el doc pa
- **[2021-11-17] Them** (0 emojis): Cuando un proceso pesado hijo es generado, el mismo hereda los descriptores de archivos de su proceso padre. Indique: (a) ¿Qué pasará en el proceso padre si el proceso hijo cierra el descriptor de archivo?. (b) ¿Qué pasará si ambos procesos, padre e hijo, escriben simultáneamente al archivo?
- **[2023-10-26] Ivan** (0 emojis): basicamente escribir documentos y verificar cosas onda el boton esta? hacer click anda?  ahora estoy hacienda cursos para automatizar como MM quiere pero por mi cuenta se hacer en python ya
- **[2022-09-23] Ivan** (0 emojis): hola que tal?  vi que hoy hay un evento de progrmacion con robots que vas a hacer que maso van a hacer? suena interesnte  tengo un proyecto que quiero hacer con robotica


## Notable messages (auto-extracted)

- **[2022-03-09] Ivan** (0 emojis): Holis Que tal?  Vos estás por poli?  Estoy aplicando a unas becas a Canadá Y pedí unos doscientos por e trámite  Y al parecer hay que retirar de la facultad y no sé cuando voy a ten cuidado tiempo de ir   Y te quiero preguntar si podes por si acaso retirar por mi y mandarme onda camscanner el doc pa
- **[2021-11-17] Them** (0 emojis): Cuando un proceso pesado hijo es generado, el mismo hereda los descriptores de archivos de su proceso padre. Indique: (a) ¿Qué pasará en el proceso padre si el proceso hijo cierra el descriptor de archivo?. (b) ¿Qué pasará si ambos procesos, padre e hijo, escriben simultáneamente al archivo?
- **[2023-10-26] Ivan** (0 emojis): basicamente escribir documentos y verificar cosas onda el boton esta? hacer click anda?  ahora estoy hacienda cursos para automatizar como MM quiere pero por mi cuenta se hacer en python ya
- **[2022-09-23] Ivan** (0 emojis): hola que tal?  vi que hoy hay un evento de progrmacion con robots que vas a hacer que maso van a hacer? suena interesnte  tengo un proyecto que quiero hacer con robotica


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Mario Gueyraud"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 703
**Chats analyzed**: 1
**Last contact**: 1005d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 60.0% | ABOVE baseline (32%) |
| Peak hour | 0h | |
| Peak day | Tuesday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 49.5% | Balanced |
| Ivan starts conv | 10 | |
| They start conv | 18 | |
| Ivan initiator % | 35.7% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 3d |
| Avg voice % | 4.1% |

### Clinical Inquiries

- What does the late-night pattern (60.0%) mean to you?
