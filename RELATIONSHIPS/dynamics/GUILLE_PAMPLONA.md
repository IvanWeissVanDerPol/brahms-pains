# Guille Pamplona

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595985733375
> **Tier:** untiered_personal · **Relationship score:** 36.7 (WARM)
> **Stats:** 85 messages · 2022-08-13 → 2023-09-12

## Overview

**Auto-extracted stats** (50 from Ivan, 35 from them, 2 emojis)

Peak months: `2022-08` (34 msgs), `2023-09` (51 msgs)
Language mix: other: 58, es: 26, en: 1
Most active: **Tuesday** at **0:00** (night (23-6))
Top topics: clase (4), capaz (4), eusebio (3), tenía (3), profe (3)
Avg msg length: Ivan 23 chars, them 46 chars
Avg reply time: Ivan 10m, them 1m
Question ratio: Ivan 12.0%, them 5.7%
Longest streak: **1** consecutive days · Longest gap: **395** days
Audio usage: 1.2% of all messages are voice notes
Sentiment: Ivan -1.00 (very negative) (pos 0 / neg 1), them 0.00 (slightly negative) (pos 0 / neg 0)
Mood trend: overall -0.50, recent -0.50, trend Δ +0.00 (0.00 (stable))

**First message** (2022-08-13, Them): Uff, estaba almorzando ahora abro el WhatsApp

**Last from Ivan** (2023-09-12): Xd

**Last from them** (2023-09-12): Esa onda

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 85 |
| First message | 2022-08-13 |
| Last message | 2023-09-12 |

## Key moments / Topics

Auto-extracted notable messages from the chat:

## Notable messages (auto-extracted)

- **[2023-09-12] Ivan** (0 emojis): holis  estoy lleyendo nomas los powerpoints random del profe pero son todos re contra mierda  tengo el drive de ale si queres Uwu  capaz te sirve mas que leer lo del profe (sus examenes eran online y eran mas densos)
- **[2023-09-12] Them** (0 emojis): No sabes de nadie que estuvo en la última clase? En ciber que tuvimos con el era que decía más o menos la onda
- **[2023-09-12] Them** (0 emojis): Y después. Che, vos no estuviste en la última clase o escuchaste algo respecto al examen de emergentes?


## Notable messages (auto-extracted)

- **[2023-09-12] Ivan** (0 emojis): holis  estoy lleyendo nomas los powerpoints random del profe pero son todos re contra mierda  tengo el drive de ale si queres Uwu  capaz te sirve mas que leer lo del profe (sus examenes eran online y eran mas densos)
- **[2023-09-12] Them** (0 emojis): No sabes de nadie que estuvo en la última clase? En ciber que tuvimos con el era que decía más o menos la onda
- **[2023-09-12] Them** (0 emojis): Y después. Che, vos no estuviste en la última clase o escuchaste algo respecto al examen de emergentes?


## Notable messages (auto-extracted)

- **[2023-09-12] Ivan** (0 emojis): holis  estoy lleyendo nomas los powerpoints random del profe pero son todos re contra mierda  tengo el drive de ale si queres Uwu  capaz te sirve mas que leer lo del profe (sus examenes eran online y eran mas densos)
- **[2023-09-12] Them** (0 emojis): No sabes de nadie que estuvo en la última clase? En ciber que tuvimos con el era que decía más o menos la onda
- **[2023-09-12] Them** (0 emojis): Y después. Che, vos no estuviste en la última clase o escuchaste algo respecto al examen de emergentes?


## Notable messages (auto-extracted)

- **[2023-09-12] Ivan** (0 emojis): holis  estoy lleyendo nomas los powerpoints random del profe pero son todos re contra mierda  tengo el drive de ale si queres Uwu  capaz te sirve mas que leer lo del profe (sus examenes eran online y eran mas densos)
- **[2023-09-12] Them** (0 emojis): No sabes de nadie que estuvo en la última clase? En ciber que tuvimos con el era que decía más o menos la onda
- **[2023-09-12] Them** (0 emojis): Y después. Che, vos no estuviste en la última clase o escuchaste algo respecto al examen de emergentes?


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Guille Pamplona"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 85
**Chats analyzed**: 1
**Tier(s)**: tier3_extended
**Last contact**: 1049d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 60.0% | ABOVE baseline (32%) |
| Peak hour | 0h | |
| Peak day | Tuesday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 58.8% | Balanced |
| Ivan starts conv | 1 | |
| They start conv | 1 | |
| Ivan initiator % | 50.0% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 1d |
| Avg voice % | 1.2% |

### Clinical Inquiries

- **HIGH late-night (60.0%)**: This contact is heavily active in Ivan's vulnerability window
- **Abandoned (1049d)**: Grief signal - over a year silent
- **Text-only (1.2%)**: Distance modality
