# Foneca

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595971612900
> **Tier:** untiered_personal · **Relationship score:** 46.5 (WARM)
> **Stats:** 135 messages · 2025-06-12 → 2025-09-04

## Overview

**Auto-extracted stats** (74 from Ivan, 61 from them, 0 emojis)

Peak months: `2025-06` (107 msgs), `2025-08` (12 msgs), `2025-09` (16 msgs)
Language mix: es: 20, other: 115
Most active: **Thursday** at **15:00** (night (23-6))
Top topics: estoy (8), https (6), github (5), examen (4), parcial (3)
Avg msg length: Ivan 20 chars, them 17 chars
Avg reply time: Ivan 15m, them 11m
Question ratio: Ivan 8.1%, them 8.2%
Longest streak: **2** consecutive days · Longest gap: **57** days
Audio usage: 2.2% of all messages are voice notes
Sentiment: Ivan +1.00 (very positive) (pos 4 / neg 0), them +1.00 (very positive) (pos 1 / neg 0)
Mood trend: overall +0.00, recent +0.00, trend Δ +0.00 (0.00 (stable))

**They call Ivan:** 'ivan' (1x)

**First message** (2025-06-12, Them): Hola Ivan, qué tal, Foneca soy :)

**Last from Ivan** (2025-09-03): X si surge 2nd job(?

**Last from them** (2025-09-03): Pasá la info

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 135 |
| First message | 2025-06-12 |
| Last message | 2025-09-04 |

## Key moments / Topics

Auto-extracted notable messages from the chat:

## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111
- **[2025-06-12] Them** (0 emojis): Por si acaso no tenés los exámenes con las respuestas del primer y segundo parcial de bd?
- **[2025-06-24] Ivan** (0 emojis): Hola Queres estudiar bd2? Irl estos dias? Antes del examen :)


## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111
- **[2025-06-12] Them** (0 emojis): Por si acaso no tenés los exámenes con las respuestas del primer y segundo parcial de bd?
- **[2025-06-24] Ivan** (0 emojis): Hola Queres estudiar bd2? Irl estos dias? Antes del examen :)


## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111
- **[2025-06-12] Them** (0 emojis): Por si acaso no tenés los exámenes con las respuestas del primer y segundo parcial de bd?
- **[2025-06-24] Ivan** (0 emojis): Hola Queres estudiar bd2? Irl estos dias? Antes del examen :)


## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111


## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111


## Notable messages (auto-extracted)

- **[2025-06-26] Ivan** (0 emojis): https://www.google.com/maps/place/Weiss+house/data=!4m2!3m1!1s0x0:0xf0c757dad8e51218?sa=X&ved=1t:2428&ictx=111


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Foneca"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 135
**Chats analyzed**: 1
**Tier(s)**: tier3_extended
**Last contact**: 326d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 52.6% | ABOVE baseline (32%) |
| Peak hour | 15h | |
| Peak day | Thursday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 54.8% | Balanced |
| Ivan starts conv | 5 | |
| They start conv | 3 | |
| Ivan initiator % | 62.5% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 2d |
| Avg voice % | 2.2% |

### Clinical Inquiries

- **HIGH late-night (52.6%)**: This contact is heavily active in Ivan's vulnerability window
- **Text-only (2.2%)**: Distance modality
