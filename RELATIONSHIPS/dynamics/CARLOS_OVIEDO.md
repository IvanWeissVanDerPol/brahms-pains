# Carlos Oviedo

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595981545498
> **Tier:** untiered_personal · **Relationship score:** 37.8 (WARM)
> **Stats:** 79 messages · 2023-07-08 → 2024-02-17

## Overview

**Auto-extracted stats** (29 from Ivan, 50 from them, 4 emojis)

Peak months: `2023-07` (3 msgs), `2023-10` (35 msgs), `2023-11` (10 msgs)
Language mix: other: 63, es: 16
Most active: **Friday** at **15:00** (afternoon (12-18))
Top topics: tengo (8), estoy (5), filamento (3), estás (3), señor (2)
Avg msg length: Ivan 13 chars, them 20 chars
Avg reply time: Ivan 22m, them 21m
Question ratio: Ivan 17.2%, them 24.0%
Longest streak: **2** consecutive days · Longest gap: **111** days
Audio usage: 1.3% of all messages are voice notes
Sentiment: Ivan 0.00 (slightly negative) (pos 0 / neg 0), them -1.00 (very negative) (pos 0 / neg 1)
Mood trend: overall +0.25, recent +0.25, trend Δ +0.00 (0.00 (stable))

**First message** (2023-07-08, Ivan): Holis

**Last from Ivan** (2024-02-17): Voy

**Last from them** (2024-02-17): Estoy afuera

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 79 |
| First message | 2023-07-08 |
| Last message | 2024-02-17 |

## Key moments / Topics

Auto-extracted notable messages from the chat:


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Carlos Oviedo"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 79
**Chats analyzed**: 1
**Tier(s)**: tier3_extended
**Last contact**: 890d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 6.3% | below baseline |
| Peak hour | 15h | |
| Peak day | Friday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 36.7% | Balanced |
| Ivan starts conv | 1 | |
| They start conv | 6 | |
| Ivan initiator % | 14.3% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 2d |
| Avg voice % | 1.3% |

### Clinical Inquiries

- **Abandoned (890d)**: Grief signal - over a year silent
- **Text-only (1.3%)**: Distance modality
