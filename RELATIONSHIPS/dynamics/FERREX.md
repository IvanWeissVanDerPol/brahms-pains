# Ferrex

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595991952375
> **Tier:** untiered_personal · **Relationship score:** 49.4 (WARM)
> **Stats:** 29 messages · 2025-06-27 → 2026-02-07

## Overview

**Auto-extracted stats** (12 from Ivan, 17 from them, 0 emojis)

Peak months: `2025-06` (5 msgs), `2026-02` (24 msgs)
Language mix: other: 20, es: 9
Most active: **Saturday** at **1:00** (night (23-6))
Top topics: delivery (2), caacupe (2), momento (2), mañana (2), estas (1)
Avg msg length: Ivan 21 chars, them 17 chars
Avg reply time: Ivan 4m, them 1m
Question ratio: Ivan 41.7%, them 5.9%
Longest streak: **2** consecutive days · Longest gap: **224** days
Audio usage: 6.9% of all messages are voice notes
Sentiment: Ivan +1.00 (very positive) (pos 1 / neg 0), them 0.00 (slightly negative) (pos 0 / neg 0)
Mood trend: overall +0.50, recent +0.50, trend Δ +0.00 (0.00 (stable))

**First message** (2025-06-27, Them): .

**Last from Ivan** (2026-02-07): okis

**Last from them** (2026-02-07): 60 mil sería así total

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 29 |
| First message | 2025-06-27 |
| Last message | 2026-02-07 |

## Key moments / Topics

Auto-extracted notable messages from the chat:


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Ferrex"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 75
**Chats analyzed**: 2
**Tier(s)**: tier3_extended, untiered_personal
**Last contact**: 170d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 32.0% | baseline |
| Peak hour | 1h | |
| Peak day | Saturday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 42.7% | Balanced |
| Ivan starts conv | 2 | |
| They start conv | 1 | |
| Ivan initiator % | 66.7% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 2d |
| Avg voice % | 2.7% |

### Clinical Inquiries

- **Text-only (2.7%)**: Distance modality
