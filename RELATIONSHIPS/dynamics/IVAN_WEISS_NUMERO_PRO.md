# Iván Weiss Número Pro

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595981324569
> **Tier:** tier1_deep · **Relationship score:** 40.9 (WARM)
> **Stats:** 663 messages · 2024-08-31 → 2026-03-14

## Overview

**Auto-extracted stats** (663 from Ivan, 0 from them, 422 emojis)

Peak months: `2024-08` (1 msgs), `2024-12` (1 msgs), `2026-01` (1 msgs)
Language mix: other: 254, es: 59, en: 347, mixed: 3
Most active: **Friday** at **1:00** (night (23-6))
Top topics: valid (244), googleapis (244), openclaw (219), github (206), message (177)
Avg msg length: Ivan 364 chars, them 0 chars
Avg reply time: Ivan n/a, them n/a
Question ratio: Ivan 19.2%, them 0.0%
Longest streak: **9** consecutive days · Longest gap: **400** days
Audio usage: 0.2% of all messages are voice notes
Sentiment: Ivan -0.11 (slightly negative) (pos 4 / neg 5), them 0.00 (slightly negative) (pos 0 / neg 0)
Mood trend: overall -0.03, recent -0.03, trend Δ +0.00 (0.00 (stable))

**Ivan calls them:** 'kiki' (4x), 'kyrian' (3x), 'amor' (2x), 'bb' (1x), 'mamá' (1x)

**First message** (2026-01-07, Ivan): Hola! Vi los precios de Vetic.  Mi clinica tiene aproximadamente 430 consultas/mes.  Me interesa el Plan Profesional.  Quiero saber mas!

**Last from Ivan** (2026-03-14): ⚠️ API rate limit reached. Please try again later.

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 663 |
| First message | 2024-08-31 |
| Last message | 2026-03-14 |

## Key moments / Topics

Auto-extracted notable messages from the chat:

## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notable messages (auto-extracted)

- **[2026-02-05] Ivan** (14 emojis): [openclaw] Here's the overnight roundup:  **🐾 Vete Autonomous Workers — Massive Progress** 11 worker sessions ran since last night. Highlights:  - ✅ **API test auth fixed** — went from ~0% to 81% pass rate - ✅ **Integration test infra stabilized** — cookies() audit errors resolved - ✅ **6 Dependabot
- **[2026-02-06] Ivan** (12 emojis): Sí, te escucho. Estaba revisando el sistema de cron.  Sobre tus pedidos:  **1. Polling adaptativo de Sarah:** El problema es que wacli no tiene acceso a indicadores de presencia/typing de Messaging (Baileys sí pero no está expuesto). Lo que SÍ puedo hacer: - Revisar si hay mensajes nuevos de ella en 
- **[2026-02-05] Ivan** (10 emojis): OK here's my full analysis of John's messages:  ---  **📊 Analysis of John's Messages in AI Whisperers Group**  **25 messages total**, all from 2026-02-05 (02:04–04:30 UTC). Here's the breakdown:  ---  **👤 Who is John** - Ivan's father, 54 years old - 35+ years dev experience - Owns company WPG, work
- **[2026-02-06] Ivan** (8 emojis): [Nyx] Done! ✅  **Sarah context provided to Erebus:**  1. **Created detailed context file:** `~/.openclaw/workspace/sarah-context.md`    - Dual-system model (Journalist armor vs Inner Child)    - 3 core wounds (Christmas abandonment, Not enough, Mother enmeshment)    - 4 defense mechanisms (Analyzer,
- **[2026-02-13] Ivan** (8 emojis): The wacli command is not working as expected. This might be a configuration issue. I'll skip group polling for now. **Step 8: Memory Maintenance** - I tried earlier but the script had an issue with OpenClaw CLI not being in PATH. Let me check if I can run it differently: Actually, looking at the hea


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Iván Weiss Número Pro"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending


---

## 📊 NEW (2026-07-27): Empirical Profile Data

**Total messages**: 17,699
**Chats analyzed**: 23
**Tier(s)**: tier1_deep, tier2_core, tier3_extended, tier4_groups, untiered_personal
**Last contact**: 8d ago

### Time Patterns

| Metric | Value | Clinical |
|--------|------:|----------|
| Late-night ratio (22:00-04:00) | 30.6% | baseline |
| Peak hour | 23h | |
| Peak day | Tuesday | |

### Initiator Dynamics

| Metric | Value | Pattern |
|--------|------:|---------|
| Ivan initiator ratio | 46.4% | Balanced |
| Ivan starts conv | 716 | |
| They start conv | 1,256 | |
| Ivan initiator % | 36.3% | |

### Engagement Metrics

| Metric | Value |
|--------|------:|
| Max streak (consecutive days) | 21d |
| Avg voice % | 13.2% |

### Clinical Inquiries

- (no notable clinical signals)
