# Viví Poli IAE

> **Auto-generated profile stub** (2026-07-23T21:05:23)
> **Source:** vCard phonebook (full export 2026-07-23)
> **JID:** 595994538574
> **Tier:** _dropped · **Relationship score:** 23.6 (DORMANT)
> **Stats:** 6 messages · 2023-11-01 → 2024-08-29

## Overview

**Auto-extracted stats** (5 from Ivan, 1 from them, 0 emojis)

Peak months: `2023-11` (1 msgs), `2024-08` (5 msgs)
Language mix: other: 4, es: 2
Most active: **Thursday** at **1:00** (night (23-6))
Top topics: https (2), youtu (2), podcast (1), jajajaja (1), sospechando (1)
Avg msg length: Ivan 29 chars, them 61 chars
Avg reply time: Ivan 8m, them 1.1h
Question ratio: Ivan 40.0%, them 0.0%
Longest streak: **1** consecutive days · Longest gap: **302** days
Audio usage: 0.0% of all messages are voice notes
Sentiment: Ivan 0.00 (slightly negative) (pos 0 / neg 0), them 0.00 (slightly negative) (pos 0 / neg 0)

**First message** (2023-11-01, Ivan): Se pone podcast

**Last from Ivan** (2024-08-29): https://youtu.be/6cVnxNwiKv8?si=P6U9-Ue11ccEoLQh

**Last from them** (2024-08-29): Jajajaja sigo sospechando que hay algo que programar, veremos

## Communication stats

| Metric | Value |
|--------|-------|
| Total messages | 6 |
| First message | 2023-11-01 |
| Last message | 2024-08-29 |

## Key moments / Topics

Auto-extracted notable messages from the chat:


## Notes

TODO: anything else Ivan has shared or knows about this person.

## Profile sources

- vCard phonebook (full export, 2026-07-23): name "Viví Poli IAE"
- Self-intro analysis: pending
- Group context: pending
- Behavioral signals: pending
